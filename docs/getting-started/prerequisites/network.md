...
NICo does not deploy or manage the route servers — they are external infrastructure that you provision separately. When route servers are enabled in siteConfig (`enable_route_servers = true`), NICo's DPU Agent configures FRR/NVUE on each DPU to peer with the route server IPs you provide (`route_servers = [...]`). The DPU creates a `routeserver` BGP peer-group with multi-hop TTL 255, external AS, and L2VPN-EVPN address family only.
