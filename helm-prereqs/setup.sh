...
echo ""
echo "  Key fields:"
echo "    global.image.repository   — ${NCX_IMAGE_REGISTRY}/nvmetal-nico"
...
#      7c. NCX postgres (simple StatefulSet — temporal + nico DBs)
#      7d. Keycloak (dev IdP)
...
if [[ -z "${NCX_REPO:-}" ]]; then
    ...
    echo "  export NCX_REPO=/path/to/<ncx-rest-repo>   # e.g. nico-rest, ncx, ncx-infra-controller-rest"
    exit 1
fi
...
NCX_HELM_CHART="${NCX_REPO}/helm/charts/nico-rest"
...
if [[ "${_ncx_reply:-Y}" =~ ^[Yy]$ ]]; then
    helm upgrade --install nico-rest "${NCX_HELM_CHART}" \
        --namespace nico-rest \
    ...
    echo "Skipped NCX REST (nico-rest). Re-run with -y or answer Y to deploy."
    echo ""
