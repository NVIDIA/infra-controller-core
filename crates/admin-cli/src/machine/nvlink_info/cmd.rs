...
pub async fn handle_nvlink_info_populate(
    ...
    let domain_uuid = nmxm_node
        ...
        .and_then(|v| v.as_str())
        .ok_or_else(|| {
            NicoCliError::GenericError("No DomainUUID found in NMX-M response".to_string())
        })?;
    ...
    for gpu_id in &gpu_id_list {
        ...
        let gpu_response = api_client
            ...
            .await
            .map_err(|e| {
                NicoCliError::GenericError(format!("Failed to fetch GPU {}: {}", gpu_id, e))
            })?;
    ...
    let nvlink_info_rpc = nicorpc::MachineNvLinkInfo {
        domain_uuid: Some(nico_uuid::nvlink::NvLinkDomainId::from(
            domain_uuid_parsed,
        )),
        gpus: gpus.clone(),
    };
