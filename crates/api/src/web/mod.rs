...
/// Translates the internal lifecycle state into a simplified tenant-facing status string.
///
/// `lifecycle.state` is a JSON-encoded controller state meant for developers and site admins.
/// End users (tenants) should only see a coarse-grained status, so this translation
/// lives at the REST API boundary rather than in the proto, which would make a purely
/// presentational concern part of the data contract.
pub(crate) fn tenant_state_from_lifecycle(
    lifecycle: Option<&forgerpc::LifecycleStatus>,
    is_deleted: bool,
) -> &'static str {
    if is_deleted {
        // Deletion is user-initiated and overrides any controller state that hasn't caught up yet.
        return "Terminating";
    }
    ...
    match lifecycle.and_then(|lc| serde_json::from_str::<ControllerState>(&lc.state).ok()) {
        Some(ControllerState::Ready) => "Ready",
        Some(ControllerState::Deleting) => "Terminating",
        Some(ControllerState::Provisioning) | None => "Provisioning",
    }
}
...
mod tests {
    ...
    fn lifecycle(state: &str) -> forgerpc::LifecycleStatus {
        forgerpc::LifecycleStatus {
    ...
    #[test]
    fn tenant_state_lifecycle_mapping() {
        // Absent or unrecognised lifecycle defaults to Provisioning.
        assert_eq!(tenant_state_from_lifecycle(None, false), "Provisioning");

        assert_eq!(
            tenant_state_from_lifecycle(Some(&lifecycle("not valid json")), false),
            "Provisioning",
        );

        assert_eq!(
            tenant_state_from_lifecycle(Some(&lifecycle(r#"{"state":"provisioning"}"#)), false),
            "Provisioning",
        );

        assert_eq!(
            tenant_state_from_lifecycle(Some(&lifecycle(r#"{"state":"ready"}"#)), false),
            "Ready",
        );

        // Any deletion substate maps to Terminating.
        assert_eq!(
            tenant_state_from_lifecycle(
                Some(&lifecycle(
                    r#"{"state":"deleting","deletion_state":{"state":"drainallocatedips","delete_at":"2026-01-01T00:00:00Z"}}"#
                )),
                false,
            ),
            "Terminating",
        ...
        // is_deleted overrides the controller state — user-initiated deletion surfaces
        // as Terminating immediately, before the state machine catches up.
        assert_eq!(
            tenant_state_from_lifecycle(Some(&lifecycle(r#"{"state":"ready"}"#)), true),
            "Terminating",
        );

        assert_eq!(tenant_state_from_lifecycle(None, true), "Terminating");
    }
}
