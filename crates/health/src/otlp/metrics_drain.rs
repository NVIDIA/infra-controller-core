...
use crate::sink::otlp::{OtlpMetricItem, OtlpMetricsQueue};
...
impl OtlpMetricsDrainTask {
    ...
    fn drain_batch(&self, batch: &mut Vec<(EventContext, OtlpMetricItem)>) {
    ...
    async fn flush(
        ...
        batch: &mut Vec<(EventContext, OtlpMetricItem)>,
    ) {
        if batch.is_empty() {
            return;
        }
