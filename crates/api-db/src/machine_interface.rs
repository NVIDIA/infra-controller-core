...
pub async fn create_host_machine_dpu_interface_proactively(
    ...
    let is_primary = existing_machine.as_ref().map(|_| false);
...
pub async fn reconcile_admin_addresses_for_host(
    ...
    // Then lock allocation state for every admin segment this host currently touches.
    // The advisory locks coordinate with the normal admin-address allocators.
    let segments = load_and_lock_admin_segments(&mut txn, &interfaces).await?;
...
/// Used to load and exclusively lock the admin segments touched by reconciliation.
async fn load_and_lock_admin_segments(
    txn: &mut Transaction<'_>,
    interfaces: &[AdminInterfaceForReconcile],
) -> DatabaseResult<Vec<NetworkSegment>> {
    let mut segment_ids = interfaces
        .iter()
    ...
    if segments.len() != segment_ids.len() {
        return Err(DatabaseError::internal(format!(
            "Loaded {} admin segments for {} referenced segment IDs",
            segments.len(),
            segment_ids.len(),
        )));
    }
